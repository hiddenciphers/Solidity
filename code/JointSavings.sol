...
contract JointSavings {
    ...
    function withdraw(uint amount, address payable recipient) public {
        ...
        // A `require` statement that checks if the `recipient` is equal to either `accountOne` or `accountTwo`.
        // The `require` statement returns the text `"You don't own this account!"` if it does not.
        require(recipient == accountOne || recipient == accountTwo, "You don't own this account!");

        // A `require` statement that checks if the `balance` is sufficient to accomplish the withdraw operation.
        // If there are insufficient funds, the text `Insufficient funds!` is returned.
        require(contractBalance >= amount, "Insufficient funds!");

        // An `if` statement to check if the `lastToWithdraw` is not equal to (`!=`) to `recipient`.
